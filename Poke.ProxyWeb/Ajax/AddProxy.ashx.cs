...
namespace Poke.ProxyWeb.Ajax
{
    ...
    /// AddUser 的摘要说明
    /// </summary>
    public class AddProxy : HandleBase
    {
        ...
        private bool addProxy(string  realname, string phone, string province, string city, string area,int totalcard)
        {
            string sql1 = "insert into game_proxy_cardlog(proxyid,card,cardremark,cardtype,createdatetime,extend_01) values(" + UserID + "," + (-500) + ",'',2,now(),'" + gameuserid + "')";
            string sql2 = "insert into game_user_log(userid,card,proxyid,logdate) values(" + uid + "," + (card) + "," + UserID + ",now())";
            string sql3 = "update game_proxy set card =" + (totalcard - 500) + "  where id=" + UserID;
